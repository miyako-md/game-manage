...
- 已知的角色池类型有 `E_CharacterGachaPoolType_Special`（特许）、`Standard`（基础）、`Beginner`（启程）和 `Joint`（辉光庆典）。`Joint` 是随 2026-05-14 开启的「辉光庆典」卡池出现的新类型，EndUID 当天补上了支持，说明新卡池会带来新的类型。今天（2026-09-24）开启的「重构寻访」按公告是一种新的寻访类型，接口里是否对应新的枚举值还不知道。解析时遇到不认识的池类型必须标成未知，不能静默丢掉。
- 请求之间要留间隔，参考工具就是为了防风控加了延迟。增量同步时，连续遇到已知的 `seqId` 就停止。token 需要做 URL 编码。
- B服：参考工具根据记录链接里的 `channel=2&subChannel=2` 判断是 B服。2026-09-26 补充调研：B服玩家在鹰角用户中心把 B服账号绑到通行证后，可以走同一条通行证链路，详见 [B服接入可行性](endfield-bilibili-server.md)。
...
3. **使用你自己浏览器里的会话**：在自己的浏览器登录 www.skland.com，把 cred、签名 token 和浏览器生成的 dId 填进本工具。这样 dId 由官方 SDK 生成，但需要从浏览器开发者工具里手动复制。项目已有先例：B站来源面板就是让用户填 `SESSDATA`。换一个客户端使用这些值能否通过校验，还没有验证过。

账号安全方面，没有查到有人因为只读查询被处罚的公开记录，但这不能说明没有风险。签到是写操作，无论选哪种都建议默认关闭。

## 公告与活动日历

资讯：B站官号“明日方舟终末地”的 UID 是 `1265652806`。在 `config.example.toml` 的 `[bilibili_sources]` 里加 `endfield = "1265652806"`，再把它加进 `public_content.MOBILE_GAMES`，就能沿用现有的采集流程。过滤规则需要调整：在本地模拟过，一篇全文版的「雪凇幽梦」版本更新说明会被判为“宣传展示”而排除。原因是正文里有“网页活动”，而“版本更新说明”这类标题又不在现有的公告标题白名单里。改法见[实施指导](endfield-implementation-guide.md)的 M3。
...
- B站官号会发布全文版本说明，例如[「春晓时」版本更新说明](https://www.bilibili.com/opus/1192064737057177641)，所以可以作为日历主来源。官网 `endfield.hypergryph.com/news` 的正文格式相同，可以作为备选来源，但目前没有找到公开的 JSON 接口。
...
- 活动日历直接做公告解析，不做手填。
- 签到能做就做，不强求。
- 需求文档的边界没有改动，所以森空岛功能（理智、练度和签到等）只走不需要伪造指纹的路径，也就是上面的方式 3 或 TNXG 的无 dId 请求；实测走不通就不做。

具体步骤见[终末地接入实施指导](endfield-implementation-guide.md)。

## 待实测清单

实测步骤和记录表格都放在实施指导的 M0 一节：

- 通行证短信是否会触发人机验证；token 的有效期和失效码。
- 寻访：官服链路能否跑通，实际能查到多少天，「重构寻访」对应什么池类型。
- B站：全文版本说明经过调整后的过滤规则，能否正常收录。
- 森空岛：TNXG 那种完全不带 dId 的请求方式能否换到 cred；用浏览器里的 cred 请求 binding、card/detail 和签到能否通过；理智数据是否实时（异环的同类数据有延迟）。
