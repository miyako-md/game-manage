...
| 事项 | 决定 |
|---|---|
| 服务器 | 先做官服（鹰角通行证账号）。B服和国际服不在本期范围；B服的方案见 [B服接入可行性](endfield-bilibili-server.md) |
| 活动日历 | 直接解析官方公告，不做手填 |
| 签到 | 能做就做，不强求；做不了就跳过 |
...
| 里程碑 | 状态 |
|---|---|
| M0 本机实测 | 未做。开发环境无法访问鹰角和B站域名；合并前后请在本机按下文逐项核对 |
| M1 骨架与登录 | 已实现：`adapters/endfield/`、`EndfieldLoginProvider`、登录服务支持第三个游戏、前端登录与图标 |
| M2 寻访记录 | 已实现：`endfield_gacha.py`（账本、可续传同步、断档）、`/api/endfield/gacha/*`、`EndfieldGachaPanel.vue` |
| M3 资讯与日历 | 已实现B站主来源：规则版本 5、`sources/endfield_notice.py`、日历详情显示结束原文。官网公告作为原生来源尚未实现，所以终末地活动暂时没有临期提醒 |
| M4 森空岛 | 未实施，仍取决于 M0 第 4 项 |

以上代码的测试都用模拟 HTTP 和官方公告原文摘录，数据口径说明见 [终末地数据说明](endfield-data.md)。
