  ...
  <div class="workbench" :class="{ 'menu-open': menuOpen }">
    ...
    <div class="workspace-body" :inert="mobileNavigation && menuOpen">
      ...
      <main id="main-content" ref="mainContent" tabindex="-1" class="workspace-main">
        ...
        <div v-show="route.page === 'accounts'" class="accounts-page"><header class="page-heading"><p class="eyebrow">CONNECTED WORLDS</p><h1>社区账号</h1><p class="page-description">连接你的游戏世界，让账号与进度自动汇聚。</p></header><LoginPanel @account-changed="onAccountChanged" /><section class="connection-note"><AppIcon name="shield" :size="24" /><div><h2>授权只保存在本机</h2><p>鸣潮与异环使用社区手机号登录，短期凭据自动续期；终末地使用鹰角通行证手机号登录，失效后需重新登录。英雄联盟通过本机客户端读取数据，无需在这里登录。</p><p>登录成功后，进入对应游戏点击「刷新数据」即可加载账号快照。</p></div></section></div>
