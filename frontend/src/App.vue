...
const calendarErrors = computed(() => Object.fromEntries(state.games.map(g => [g.game_id, [state.readErrors[g.game_id], state.refreshErrors[g.game_id]].filter(Boolean).join('；')])))
let timer
  ...
  <div class="workbench" :class="{ 'menu-open': menuOpen }">
    ...
    <div class="workspace-body" :inert="mobileNavigation && menuOpen">
      ...
      <main id="main-content" ref="mainContent" tabindex="-1" class="workspace-main">
        ...
        <OverviewPage v-if="route.page === 'overview'" :games="state.games" :snapshots="state.snapshots" :accounts="state.accounts" :read-errors="state.readErrors" :refresh-errors="state.refreshErrors" :refreshing="state.refreshing" :now="now" :loading="state.loading" @navigate="navigate" @refresh="refreshAll" />
        ...
        <template v-else-if="route.page === 'game'"><GameCard v-if="selectedGame" :key="`${selectedGame.game_id}:${accountRevisions[selectedGame.game_id] || 0}`" :game="selectedGame" initial-section="overview" :external-snapshots="state.snapshots[selectedGame.game_id] || {}" :external-refreshing="!!state.refreshing[selectedGame.game_id]" :external-error="state.refreshErrors[selectedGame.game_id] || state.readErrors[selectedGame.game_id] || ''" @refresh="dashboard.refresh(selectedGame.game_id)" @calendar="navigate('calendar', selectedGame.game_id)" /><div v-else class="empty-page"><p>{{ state.loading ? '正在读取游戏列表…' : '这个游戏尚未启用或已从配置中移除。' }}</p><button class="ui-button" @click="navigate('overview')">返回总览</button></div></template>
