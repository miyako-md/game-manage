...
export function calendarRange(anchor = Date.now(), mode = 'month') {
  ...
  const start = mode === 'month' ? Date.UTC(local.getUTCFullYear(), local.getUTCMonth(), 1) - BEIJING_OFFSET : day
  const end = mode === 'month' ? Date.UTC(local.getUTCFullYear(), local.getUTCMonth() + 1, 1) - BEIJING_OFFSET : start + 14 * DAY_MS
...
export function eventGeometry(event, range) {
  const start = parseBeijingTime(event.start_at)
  const end = parseBeijingTime(event.end_at)
  const base = { start, end, left: 0, width: 0, visible: false, clippedStart: false, clippedEnd: false }
...
export function collectCalendarEvents(games = [], snapshots = {}, gameId = '') {
  return games.filter((game) => !gameId || gameId === 'all' || game.game_id === gameId).flatMap((game) => {
    ...
    return snap.payload.filter((event) => event && typeof event === 'object').map((event, index) => ({
      ...
      fetchedAt: snap.fetched_at, stale: Boolean(snap.stale),
    }))
  })
}
...
export function groupCalendarEvents(events) {
  ...
  for (const event of events) {
    ...
    if (!groups.has(key)) groups.set(key, { key, gameId: event.gameId, gameName: event.gameName, category: event.category, events: [] })
