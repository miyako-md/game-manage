...
// 结果未知的对局不算负场：胜率只按有胜负的场次算，条里单独一段灰色。
// 旧快照没有 losses，按「非胜即负」兜底（与当时的口径一致）。
const topChampions = computed(() => {
  const list = payload.value?.top_champions ?? []
  return list.map((c) => {
    const losses = c.losses ?? Math.max(0, (c.games || 0) - (c.wins || 0))
    const unknown = Math.max(0, (c.games || 0) - (c.wins || 0) - losses)
    const decided = (c.wins || 0) + losses
    return { ...c, losses, unknown, winrate: decided > 0 ? Math.round(((c.wins || 0) / decided) * 100) : null }
  })
})
const anyUnknown = computed(() => topChampions.value.some((c) => c.unknown > 0))
    ...
    <template v-else>
      <div class="stats-body">
        ...
        <div v-if="topChampions.length > 0" class="block">
          <p class="block-title">常用英雄 <span class="legend-inline"><i class="w"></i>胜 <i class="l"></i>负<template v-if="anyUnknown"> <i class="u"></i>未知</template></span></p>
          <ul class="champions">
            <li v-for="c in topChampions" :key="c.champion_id">
              <span class="name">{{ c.champion_name || `英雄 #${c.champion_id}` }}</span>
              <span class="wl-track" :title="`${c.games} 场 ${c.wins} 胜 ${c.losses} 负${c.unknown ? ` ${c.unknown} 场结果未知` : ''}`">
                <span class="wl-fill" :style="{ width: `${(c.games / maxGames) * 100}%` }">
                  <i v-if="c.wins" class="w" :style="{ flex: c.wins }"></i><i v-if="c.losses" class="l" :style="{ flex: c.losses }"></i><i v-if="c.unknown" class="u" :style="{ flex: c.unknown }"></i>
                </span>
              </span>
...
.u { background: color-mix(in srgb, var(--text-faint) 55%, transparent); }
