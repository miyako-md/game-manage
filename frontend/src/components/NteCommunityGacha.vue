  ...
  <section class="community-stats" aria-label="社区抽卡统计">
    ...
    <template v-else>
      ...
      <section v-if="pools.length" class="history-panel" aria-label="已出 S 明细">
        <nav class="pool-tabs" aria-label="选择抽卡统计卡池"><button v-for="(pool, index) in pools" :key="index" :aria-pressed="selected === index" @click="selected = index">{{ pool.name || '未命名卡池' }}</button></nav>
