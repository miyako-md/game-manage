...
**2026-09-14 工作台 UI 重设计**：采用深蓝金色主题与侧栏导航。

- **今日总览**（`#/`）：三个游戏的账号/体力或近期胜率、临期活动、公告资讯；只读取真实本地快照。
- **游戏档案**（`#/game/<game_id>`）：全宽展示，分为概览、角色与探索、近期对局、抽卡统计、公告与资讯，按游戏能力显示；保留异环专属数据口径。
- **活动日历**（`#/calendar`）：独立横向日期轴，按游戏与活动分类绘制真实起止时间条；支持整月/14天、翻页、回到今天、筛选和详情。`#/calendar?game=nte` 可直接打开对应游戏。缺少一侧日期只显示已知日期标记，无日期或日期异常单独列出；全部使用北京时间。
- **社区账号**（`#/accounts`）：原有短信登录、人工验证、续期和退出流程；顶部账号按钮同样可进入。

首页每60秒读取本地快照；点击「刷新数据」才主动触发采集。读取失败保留已有数据并提示，切换账号会清除旧私人摘要，阻止在途旧响应重新出现。窄屏导航支持键盘操作，日历可横向滚动并固定日期头。

活动条使用数据源中的标题、分类与时间；现有 `events` 数据不提供活动横幅图，因此当前使用游戏色和分类纹理展示。
...
异环支持账号概览 / 体力与日常 / 角色练度与详情 / 成就 / 探索 / 抽卡统计 / 社区名片，
并提供匿名公告和活动日历（活动以手填为主、自动扫描兜底）；
...
（鸣潮 8 个：account / stamina / events / progress / announcement / exploration / calabash / roles；英雄联盟 5 个：account / match / stats / announcement / news；异环 9 个：account / stamina / roles / progress / exploration / gacha / record / events / announcement）。
...
| 账号（account） | 等级、世界/大亨等级、活跃天数、角色/成就/房产/载具数量 | 页面短信登录 |
| 体力（stamina） | 本性像素、都市活力、日常活跃、周本剩余（上游有提供时） | 页面短信登录 |
| 角色（roles） | 品质、等级、元素、觉醒、混频、羁遇经验；展开属性、弧盘和技能 | 页面短信登录 |
| 进度（progress） | 成就总览、奖牌数量、分类完成度 | 页面短信登录 |
| 探索（exploration） | 地区探索数量、完成度和子项明细 | 页面短信登录 |
| 抽卡（gacha） | 分池抽数、出S数量、平均出S抽数、已出S明细 | 页面短信登录 |
| 名片（record） | 社区绑定角色名片，空列表显示暂无 | 页面短信登录 |
...
联调校准）；
账号数据已标准化并接入专属卡片，2026-09-14 使用实际登录态校准过所有七类私人查询。
`roleHome` 已提供体力字段，旧“无体力”判断不再适用。抽卡统计是社区返回窗口，
已出S明细不包含当前垫抽，不能据此计算当前保底；未返回的周本次数和探索进度显示未提供/未知。
字段、来源与验收细节见 [异环展示说明](docs/nte-data.md)。
...
5. 重启后端，在页面点击刷新（或等待轮询）加载账号数据。推荐直接使用页面短信登录。

**只拿到 access_token 也先填**：查询能力立即可用；access_token 过期后相关
能力会报"塔吉多会话已失效，请重新抓取 token"，重新抓一次即可。同时填了
`nte_refresh_token` 时，后端会在会话失效后换取新令牌对并保存；刷新令牌失效时需重新登录。

`authorization` 等同账号登录凭据，**不要分享给任何人**（含截图脱敏）。

### 已知边界

- **体力来自 roleHome**：默认每 5 分钟拉取本性像素及都市活力，不猜测恢复速度或恢复满时间；
...
- 社区名片不是对局战绩；接口未下发名片时显示暂无。房产/载具本轮提供数量概览，未实现单项详情。
