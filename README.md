...
鸣潮、异环以官方 B 站动态为主来源，社区公告作为补充和回退。此规则不用于 LOL。模板已包含以下公开账号，UID是游戏官方账号，不是用户自己的账号：
...
| 配置 | 默认值／用途 |
| --- | --- |
| `wuwa_enabled` / `nte_enabled` / `lol_enabled` | 是否启用对应游戏 |
...
更多记录：[登录说明](docs/community-login.md) · [运行管理](docs/local-runtime.md) · [改动汇总](docs/changes-2026-09-16.md) · [分支与对话核查](docs/branch-audit-2026-09-16.md)。`docs/superpowers/`保存历史设计与实施过程，历史阶段描述不代表当前使用步骤。
