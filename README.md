...
| 异环 | 账号、本性像素、都市活力、日常进度、角色详情、成就、探索、抽卡统计、社区名片、公告与活动日历 | 看板「社区账号」中短信登录塔吉多 |
...
| 异环抽卡 | 社区返回的卡池统计及出S明细 | 不含完整垫抽记录，不能据此推算当前保底 |
...
2026-09-16完整验证记录：后端558项通过、4项条件跳过，前端77项通过，生产构建成功。跳过项包含需要显式启用的运行管理测试及Windows符号链接权限用例；测试通过不等同于所有上游接口持续可用。
...
- 图片OCR自动入库、房产／载具单项详情、完整抽卡历史导入、伤害计算尚未作为产品功能交付。
