...
async def test_serverchan_send():
    ...
    assert route.called
    assert parse_qs(route.calls.last.request.content.decode()) == {"title": ["标题"], "desp": ["内容"]}
...
async def test_pushplus_send():
    import json
    from game_assistant.notify.wechat_push import WeChatPushNotifier
    route = respx.post("https://www.pushplus.plus/send").mock(
    ...
    assert json.loads(route.calls.last.request.content) == {
        "token": "K", "title": "t", "content": "b", "template": "txt"
    }
